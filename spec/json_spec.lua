...
describe("json", function()
  ...
  it("decodes JSON null as json.null inside an object, preserving the key", function()
    local result = json.decode('{"a":null,"b":1}')
    assert.are.equal(json.null, result.a)
    assert.are.equal(1, result.b)
  end)

  it("decodes JSON null as json.null inside an array, preserving length and order", function()
    local result = json.decode("[1,null,3]")
    assert.are.equal(1, result[1])
    assert.are.equal(json.null, result[2])
    assert.are.equal(3, result[3])
  end)

  it("round-trips json.null back to the JSON null literal", function()
    local encoded = json.encode({ a = json.null, b = 1 })
    assert.is_true(encoded == '{"a":null,"b":1}' or encoded == '{"b":1,"a":null}')
  end)
end)
